...
def load_image_label_from_xml(img_name, data_root):
    from xml.dom import minidom
    el_list = minidom.parse(os.path.join(data_root, ANNOT_FOLDER_NAME,img_name + '.xml')).getElementsByTagName('name')
...
def load_image_label_list_from_xml(img_name_list, data_root):
    return [load_image_label_from_xml(img_name, data_root) for img_name in img_name_list]
...
class ClsDataset(ImageDataset):
    ...
    def __init__(self, img_name_list_path, data_root, transform=None):
        ...
        #self.label_list = load_image_label_list_from_xml(self.img_name_list, self.data_root)
