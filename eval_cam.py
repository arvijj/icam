...
def do_python_eval(predict_folder, gt_folder, crf, name_list, dataset, input_type='png', printlog=False):
    ...
    def compare(start, step, TP, P, T, input_type):
        for idx in range(start, len(name_list), step):
            ...
            if input_type == 'png':
                if crf is not None:
                    predict_file = os.path.join(predict_folder, 'crf', 'alpha_' + crf, '%s.png'%name)
                else:
                    predict_file = os.path.join(predict_folder, 'cam', '%s.png'%name)
                predict = np.array(Image.open(predict_file)) #cv2.imread(predict_file)
            elif input_type == 'npy':
                if crf is not None:
                    predict_file = os.path.join(predict_folder, 'crf', 'alpha_' + crf, '%s.npy'%name)
                else:
                    predict_file = os.path.join(predict_folder, 'cam', '%s.npy'%name)
                ...
                if len(list(predict_dict.values())) == 0:
                    ...
                    if crf is not None:
                        ...
                        tensor[0, :, :] = 1. - np.sum(tensor, axis=0)
    ...
    p_list = []
    for i in range(32):
        p = multiprocessing.Process(target=compare, args=(i, 32, TP, P, T, input_type))
        p.start()
        p_list.append(p)
    for p in p_list:
        p.join()
...
if __name__ == '__main__':
    ...
    loglist = do_python_eval(args.predict_dir, args.gt_dir, args.crf, name_list, args.dataset, args.type, printlog=True)
