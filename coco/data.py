...
ANNOT_FOLDER_NAME = "SegmentationClass"

CAT_LIST = ['person', 'bicycle', 'car', 'motorbike', 'aeroplane', 'bus',
...
def load_image_label_from_ann(img_name, data_root):
    ann_img = cv2.imread(os.path.join(data_root, ANNOT_FOLDER_NAME, img_name + '.png'))
    unique = np.unique(ann_img)
    multi_cls_lab = np.zeros((80), np.float32)
    for un in unique:
        if un != 0:
            multi_cls_lab[un-1] = 1.0
    return multi_cls_lab

def load_image_label_list_from_anns(img_name_list, data_root):
    return [load_image_label_from_ann(img_name, data_root) for img_name in img_name_list]

def load_image_label_list_from_npy(img_name_list, data_root):
    cls_labels_dict = np.load(os.path.join(data_root, 'cls_labels.npy'), allow_pickle=True).item()
...
def load_img_name_list(dataset_path):
    ...
    img_name_list = [img_gt_name.split(' ')[0][-16:-4] for img_gt_name in img_gt_name_list]
    #img_name_list = img_gt_name_list
    return img_name_list

class ImageDataset(Dataset):

    def __init__(self, img_name_list_path, data_root, transform=None):
    ...
    def __getitem__(self, idx):
        ...
        img = PIL.Image.open(get_img_path(name, self.data_root)).convert("RGB")
        if self.transform:
...
class ClsDataset(ImageDataset):
    ...
    def __init__(self, img_name_list_path, data_root, transform=None):
        super().__init__(img_name_list_path, data_root, transform)
        self.label_list = load_image_label_list_from_npy(self.img_name_list, data_root)
        #self.label_list = load_image_label_list_from_anns(self.img_name_list, self.data_root)
...
class AffDataset(ImageDataset):

    def __init__(self, img_name_list_path, label_la_dir, label_ha_dir, cropsize, data_root, radius=5,
...
class AffGtDataset(ImageDataset):

    def __init__(self, img_name_list_path, label_dir, cropsize, data_root, radius=5,
                 joint_transform_list=None, img_transform_list=None, label_transform_list=None):
        super().__init__(img_name_list_path, data_root, transform=None)
