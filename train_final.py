...
def train_net(cfg):
    ...
    with tqdm(total=max_itr) as pbar:
        for epoch in range(cfg.TRAIN_MINEPOCH, max_epoch):
            for i_batch, sample in enumerate(dataloader):
                ...
                pred1, cam = net(inputs.to(0))

                # Compute feature similarity loss
                n, c, h, w = cam.size()
                img1_s = F.interpolate(inputs, (h, w), mode='bilinear', align_corners=True).cuda(non_blocking=True)
                ...
                loss_fs = probutils.feature_similarity_loss(img1_s, cam, args.fsl_mu, args.fsl_sigma)
                
                loss = criterion(pred1, seg_label.to(0)) + 0.5*loss_fs
                loss.backward()
                optimizer.step()
    ...
    save_path = os.path.join(cfg.MODEL_SAVE_DIR, 'final.pth')
    torch.save(parameter_source.state_dict(),save_path)
    if cfg.TRAIN_TBLOG:
        tblogger.close()
    print('%s has been saved'%save_path)
